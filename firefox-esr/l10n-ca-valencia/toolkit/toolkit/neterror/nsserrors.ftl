...
psmerr-hostreusedissuerandserial =
    Heu rebut un certificat que no és vàlid. Contacteu amb l'administrador del servidor o amb el remitent de correu electrònic i doneu-los esta informació:
...
sec-error-krl-bad-signature = La KRL del certificat d'este lloc no té una signatura vàlida.
...
sec-error-pkcs12-invalid-mac = No s'ha pogut importar. La MAC no és vàlida. La contrasenya és incorrecta o el fitxer està malmés.
...
sec-error-krl-not-yet-valid = La llista de revocació de claus d'este certificat encara no és vàlida.
sec-error-crl-not-yet-valid = La llista de revocació de certificats d'este certificat encara no és vàlida.
...
mozilla-pkix-error-ca-cert-used-as-end-entity = El servidor utilitza un certificat amb una extensió de restriccions bàsiques que l'identifica com a entitat certificadora. Això no hauria de passar amb un certificat que s'haja emés correctament.
...
mozilla-pkix-error-additional-policy-constraint-failed = En validar este certificat, ha fallat una restricció de política addicional.
